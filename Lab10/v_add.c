...
v_add_optimized_adjacent (double *x, double *y, double *z)
{
  #pragma omp parallel
  {
    ...
    for (int i = 0; i < ARRAY_SIZE; i++)
    {
      if ((i+1)%(thread_ID+1)==0)//1-arraysize,1-num of thread
      {
        z[i] = x[i] + y[i];
      }
...
v_add_optimized_chunks (double *x, double *y, double *z)
{
  #pragma omp parallel
  {
    ...
    int chunk_size=ARRAY_SIZE/thread_num; //1000 / 10 = 100
    int res_size=ARRAY_SIZE%thread_num; //add to the last thread
    int begin=chunk_size*thread_ID; //100*0=0,0-99(<100),100-199,...,900-999(<1000)+re_size
    ...
    {
      for (int i = begin+chunk_size; i < begin+chunk_size+res_size; i++)
      {
        z[i] = x[i] + y[i];
      }
    }
