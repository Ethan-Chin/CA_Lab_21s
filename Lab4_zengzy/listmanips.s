...
.text
main:	jal	create_default_list # ra point to here when call jal, tag 2
	add	s1, a0, zero	# s1=a0 is head of node list, a0 point to the last added node(iq)
...
create_default_list:
	...
	li	s2, 0		# number of nodes handled,i 
loop:					#do...
	li	a1, 8
	jal	malloc		# get memory for the next node, 1 jump point
	sw	s2, 0(a0)		# node->value = i, update *a0, there are 8 byte start in a0
	sw	s1, 4(a0)		# node->next = last
	add	s1, zero, a0	# last = node
	addi	s2, s2, 1		# i++
    	addi    t1, s2, -10
	bne	t1, zero, loop	# ... while i!= 10
	lw	ra, 0(sp) # done create_default_list
	addi	sp, sp, 4
	jr	ra # jump to tag 2
...
malloc:	# store 8 byte in heap and return pointer to it in a0
	li	a0, 9
	ecall
	jr	ra # jump to 1
