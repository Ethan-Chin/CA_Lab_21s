...
int sum_vectorized( int n, int *a )
{
    ...
    for( int i = 0; i < n/4*4; i += 4)
    {
        a1 = _mm_loadu_si128((__m128i*)(a+i));
        sum1 =  _mm_add_epi32(sum1 , a1);
    }

    int sum = ((int*)&sum1)[0]+((int*)&sum1)[1]+((int*)&sum1)[2]+((int*)&sum1)[3];
...
int sum_vectorized_unrolled( int n, int *a )
{
    ...
    for (int i = 0;i < n/16*16;i+=16)
    {
        a1 = _mm_loadu_si128((__m128i*)(a+i));
        sum1 =  _mm_add_epi32(sum1 , a1);
        a2 = _mm_loadu_si128((__m128i*)(a+i+4));
        sum1 =  _mm_add_epi32(sum1 , a2);
        a3 = _mm_loadu_si128((__m128i*)(a+i+8));
        sum1 =  _mm_add_epi32(sum1 , a3);
        a4 = _mm_loadu_si128((__m128i*)(a+i+12));
        sum1 =  _mm_add_epi32(sum1 , a4);
    }

    int sum = ((int*)&sum1)[0]+((int*)&sum1)[1]+((int*)&sum1)[2]+((int*)&sum1)[3];
    
    for ( int i = n/16*16; i < n; i++) sum += a[i];
