...
void vector_set(vector_t *v, size_t loc, int value) {
    ...
    /* If the requested location is higher than we have allocated, resize the data.*/
    if (loc >= v->size) {
        int new_size = (loc+1)*2;
        v->data = (int*) realloc(v->data, new_size*sizeof(int));
        int i;
        for (i = v->size; i < new_size; i++) v->data[i] = 0;
        v->size = new_size;
    }

    /* Set the location with value properly*/
    v->data[loc] = value;
}